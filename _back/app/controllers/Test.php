...
class Test extends Controller
{
    ...
    private function scheduleRounds($rounds, $sessionStart, $sessionEnd, $roundDuration, $workingHours)
    {
        $scheduledRounds = [];
        ...
        foreach ($rounds as $round => $pairs) {
            ...
            while (
                ...
                $this->isDuringLunchBreak($roundStartTime, $roundEndTime, $workingHours)
            ) {
                // If the round extends beyond the session end time or falls during lunch break, move to the next day
                $roundStartTime = $this->getNextWorkingDayStart($roundStartTime, $workingHours);
                $roundEndTime = $roundStartTime + $roundDuration;
            }

            if ($roundEndTime > $sessionEndTime) {
                // If the round still extends beyond the session end time, adjust the end time
                $roundEndTime = $sessionEndTime;
            }
    ...
    private function isDuringLunchBreak($roundStart, $roundEnd, $workingHours)
    {
    ...
    private function getNextWorkingDayStart($timestamp, $workingHours)
    {
        $nextDayTimestamp = strtotime('+1 day', $timestamp);

        while (!$this->isWorkingDay($nextDayTimestamp, $workingHours)) {
            $nextDayTimestamp = strtotime('+1 day', $nextDayTimestamp);
        }

        return strtotime($workingHours[0]['start'], $nextDayTimestamp);
    }

    private function isWorkingDay($timestamp, $workingHours)
    {
    ...
    public function createSession()
    {
        ...
        $roundDuration = 65 * 60; // 60 minutes in seconds

        // Schedule the rounds
        $scheduledRounds = $this->scheduleRounds($rounds, $sessionStart, $sessionEnd, $roundDuration, $workingHours);
